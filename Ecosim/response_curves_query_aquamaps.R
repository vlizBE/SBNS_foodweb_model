...
### Adjusted for SBNS food web model
...
library("devtools")
install_bitbucket("bklamer/rcrypt", force = TRUE)
...
fg         <- read.csv("C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/speciesListGoM_raw_MS.csv") ## 'fg' represents Ecospace functional groups
...
write.csv(fg, "C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/speciesListGoM_QAQC_AMkeys_MS.csv", row.names = F)
...
for (i in 1:nrow(long_fg)) {
  ...
  prf <- prf %>% 
    select(LifeStage,   Pelagic,
           ...
           PrimProdMin, PrimProdPrefMin, PrimProdPrefMax, PrimProdMax,
...
# minor addition to avoid col name issue
colnames(pref_bysp)[colnames(pref_bysp) == 'EwE_num.x'] <- 'EwE_num'
...
write.csv(fg_pref, "C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/fg-env-preference-parameters_MS_WITHPP.csv", row.names = FALSE)
