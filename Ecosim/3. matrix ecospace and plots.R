...
## Setup -----------------------------------------------------------------------
rm(list=ls()); gc()
source("C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/2. log function.R")
fg_pref <- read.csv("C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/fg-env-preference-parameters-adjusted_MS.csv")
dir_out <- "C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/output-for-Ecospace/"
...
## -----------------------------------------------------------------------------
## Primary productivity (addition MS)
driver = "PrimProd"
range = "nar"
max = 130
n_steps = 1200
...
for (i in 1:ncol(pref_mat)){
  #  i = 2
  absmin = fg_pref$PrimProdMin[i]
  prfmin = fg_pref$PrimProdPrefMin[i]
  prfmax = fg_pref$PrimProdPrefMax[i]
  absmax = fg_pref$PrimProdMax[i]
...
for (i in 1:ncol(pref_mat)){
  #  i = 2
  absmin = fg_pref$OxyMin[i]
  prfmin = fg_pref$OxyPrefMin[i]
  prfmax = fg_pref$OxyPrefMax[i]
  absmax = fg_pref$OxyMax[i]
...
for (i in 1:ncol(pref_mat)){
  #  i = 2
  absmin = fg_pref$LandDistMin[i]
  prfmin = fg_pref$LandDistPrefMin[i]
  prfmax = fg_pref$LandDistPrefMax[i]
  absmax = fg_pref$LandDistMax[i]
...
dir_pdf = "C:/Users/martha.stevens/OneDrive - VLIZ/Documenten/ECOPATH/Response curves - Holden Harris/Rscripts Holden Harris/figures/"
n_rows = 6 ## Number of plots per row on a page
n_cols = 6 ## Number of plots per column on a page
pg_plts = n_rows * n_cols
...
for(i in seq(from = 1, to = n_plots, by = pg_plts)) {
  ...
  for(j in 1:26) { # 1:26 instead of original code to fix issue
    ## Plot depth
